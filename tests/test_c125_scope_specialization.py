...
#: The count key and warning prefix the tip stamps when a conflict is withdrawn.
#: Resolved through ``getattr`` so that a BASE-TREE run of this file fails on
#: VALUES rather than on a missing attribute (G9): at ``4d417e4f`` neither name
#: exists on the driver, and neither key is ever written, so every REGRESSION
#: GUARD that asserts their ABSENCE must still be able to run there.
COUNT_SPECIALIZATIONS = getattr(
    driver, "COUNT_SCOPE_SPECIALIZATIONS", "stage0_scope_specializations"
)
WARN_SPECIALIZATION_PREFIX = getattr(
    driver, "WARN_SCOPE_SPECIALIZATION_PREFIX", "stage-0 scope specialization admitted: "
)
...
def test_g9_base_failure_a_narrower_fumonisin_reading_lets_the_run_proceed() -> None:
    ...
    assert warning.startswith(WARN_SPECIALIZATION_PREFIX)
    assert FUMONISIN_STAGE0 in warning and FUMONISIN_REQUESTED in warning
...
def test_new_capability_the_rule_is_not_a_substring_test() -> None:
    ...
    assert sorted(verdict.observed_kinds) == ["biosynthesis", "inhibition"]
