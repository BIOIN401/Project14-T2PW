...
class DeterministicPwmlBuilder:
    ...
    def _populate_sections_from_ir(self) -> Dict[str, int]:
        ...
        # Map from (compound_id, state_key, consumer_rxn_idx) -> producer_rxn_idx, indicating
        # that the consumer's left-side location for this compound should reuse the
        # producer's right-side location for the same compound/state.
        shared_from_previous: Dict[Tuple[int, str, int], int] = {}

        # Maximum "row" distance (in the adaptive reaction grid) between a producer and a
        # consumer reaction within the same biological state for the shared location to be
        # considered geometrically safe to reuse. Reactions further apart than this are
        # likely to be in visually distant parts of the diagram, so duplicating the
        # compound near each of them avoids long/ambiguous edges.
        _MAX_ROW_DISTANCE_FOR_REUSE = 1

        # Maximum "col" distance (in the adaptive reaction grid) between a producer and a
        # consumer reaction for the shared location to be considered geometrically safe to
        # reuse. Combined with _MAX_ROW_DISTANCE_FOR_REUSE, this restricts reuse to
        # grid-adjacent cells (including diagonal neighbors), preventing very long edges
        # that would otherwise connect a reused location to a reaction placed several
        # columns away.
        _MAX_COL_DISTANCE_FOR_REUSE = 1

        def _reaction_row_col(reaction_key: str) -> Optional[Tuple[int, int]]:
            layout = reaction_layout_by_key.get(reaction_key)
            if not layout:
                return None
            return int(layout.get("row", 0)), int(layout.get("col", 0))

        def _geometrically_safe_to_reuse(producer_key: str, consumer_key: str) -> bool:
            producer_rc = _reaction_row_col(producer_key)
            consumer_rc = _reaction_row_col(consumer_key)
            if producer_rc is None or consumer_rc is None:
                # If layout info is unavailable for either reaction, fall back to allowing
                # reuse (consecutive-index reuse already worked without this check).
                return True
            row_distance = abs(producer_rc[0] - consumer_rc[0])
            col_distance = abs(producer_rc[1] - consumer_rc[1])
            return (
                row_distance <= _MAX_ROW_DISTANCE_FOR_REUSE
                and col_distance <= _MAX_COL_DISTANCE_FOR_REUSE
            )

        for identity in sorted(participant_occurrences):
            compound_id, state_key = identity
            compound_record = compound_record_by_id.get(compound_id, {})
            producers = right_occurrences.get(identity, [])
            consumers = left_occurrences.get(identity, [])
            debug_base = {
            ...
            if _is_currency_compound_record(compound_record):
                for producer_idx, producer_key, _side, _member_key in producers:
                    for consumer_idx, consumer_key, _cside, _cmember_key in consumers:
                        if consumer_idx == producer_idx:
                            continue
                        self.layout_debug_shared_intermediates.append(
                            {
                                **debug_base,
                                "action": "skipped_cofactor",
                                "producer_rxn_idx": producer_idx,
                                "producer_reaction_key": producer_key,
                                "consumer_rxn_idx": consumer_idx,
                                "consumer_reaction_key": consumer_key,
                            }
                        )
                continue

            # A producer/consumer pair is eligible for reuse only when the consumer comes
            # strictly after the producer in the reaction sequence (forward connections are
            # preferred to avoid back-edges that would be confusing to read), and they are
            # not the same reaction (a reaction consuming and producing the same compound
            # is not a "shared intermediate" in the relevant sense).
            eligible_pairs: List[Tuple[Tuple[int, str, str, str], Tuple[int, str, str, str]]] = []
            for producer in producers:
                producer_idx, producer_key, _pside, _pmember_key = producer
                for consumer in consumers:
                    consumer_idx, consumer_key, _cside, _cmember_key = consumer
                    if consumer_idx == producer_idx:
                        continue
                    if consumer_idx < producer_idx:
                        continue
                    eligible_pairs.append((producer, consumer))

            if not eligible_pairs:
                continue

            # Determine the distinct producers/consumers participating in eligible pairs.
            distinct_producers = {pair[0][:2] for pair in eligible_pairs}
            distinct_consumers = {pair[1][:2] for pair in eligible_pairs}

            if len(distinct_producers) > 1 or len(distinct_consumers) > 1:
                # Multiple producers and/or multiple consumers for this compound/state: this
                # is a hub/branch point. Forcing reuse for all of them would create
                # ambiguous geometry (one location feeding/being fed by many edges in
                # different directions), so we leave these duplicated.
                for producer, consumer in eligible_pairs:
                    producer_idx, producer_key = producer[0], producer[1]
                    consumer_idx, consumer_key = consumer[0], consumer[1]
                    self.layout_debug_shared_intermediates.append(
                        {
                            **debug_base,
            ...
            # Exactly one producer and one consumer for this compound/state among eligible
            # pairs: a non-branching linear chain (possibly spanning more than one hop).
            (producer_idx, producer_key, _pside, _pmember_key), (
                consumer_idx,
                consumer_key,
                _cside,
                _cmember_key,
            ...
            if not _geometrically_safe_to_reuse(producer_key, consumer_key):
                self.layout_debug_shared_intermediates.append(
                    {
                        **debug_base,
                        "action": "skipped_geometry",
                        "producer_rxn_idx": producer_idx,
                        "producer_reaction_key": producer_key,
                        "consumer_rxn_idx": consumer_idx,
                        "consumer_reaction_key": consumer_key,
            ...
            shared_from_previous[(compound_id, state_key, consumer_idx)] = producer_idx
            action = "detected_shared_intermediate" if consumer_idx == producer_idx + 1 else (
                "detected_shared_intermediate_nonadjacent"
            )
            self.layout_debug_shared_intermediates.append(
                {
                    **debug_base,
        ...
        self.layout_debug_counts.update(
            {
                "shared_intermediates_detected": sum(
                    ...
                    if item.get("action")
                    in ("detected_shared_intermediate", "detected_shared_intermediate_nonadjacent")
                ),
