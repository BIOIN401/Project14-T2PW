...
def _mapped_identities(row: Dict[str, Any], preferred_mapped_keys: Sequence[str]) -> List[Tuple[str, str]]:
    mapped = _safe_dict(row.get("mapped_ids"))
    identities: List[Tuple[str, str]] = []
    for key in preferred_mapped_keys:
        value = mapped.get(key) or row.get(key)
        values = value if isinstance(value, list) else [value]
        for item in values:
            text = str(item or "").strip()
            if text:
                identities.append((key, text.casefold()))
    return identities
...
def _dedupe_entity_rows(
    ...
    *,
    ...
    """Return alias-aware entity rows keyed by each original entity name.

    Rows sharing the same preferred mapped identifier are represented by the same
    merged dict. The first row's name remains canonical while mapped IDs are
    unioned from later aliases.
    """
    ...
    for row in rows:
        ...
        if not name:
            continue
        identities = _mapped_identities(row, preferred_mapped_keys)
        target = next((by_identity[identity] for identity in identities if identity in by_identity), None)
        if target is None:
            target = deepcopy(row)
            target["name"] = name
            target["mapped_ids"] = dict(_safe_dict(row.get("mapped_ids")))
            target["_dedupe_aliases"] = [name]

        aliases = target.setdefault("_dedupe_aliases", [])
        if name not in aliases:
            aliases.append(name)
        target_mapped = target.setdefault("mapped_ids", {})
        for key, value in _safe_dict(row.get("mapped_ids")).items():
            if key not in target_mapped or not target_mapped.get(key):
                target_mapped[key] = value
...
def sbml_species_id(row: Dict[str, Any], compartment_id: str) -> str:
    kind = str(row.get("kind") or row.get("type") or "").strip().casefold()
    prefix = "p" if kind == "protein" else "pc" if kind in {"complex", "protein_complex"} else "m"
    mapped = _safe_dict(row.get("mapped_ids"))
    identifier = next(
        (
            str(mapped[key]).strip()
            for key in ["uniprot", "chebi", "hmdb", "kegg", "pubchem", "drugbank"]
            if str(mapped.get(key) or "").strip()
        ),
        "",
    )
    if not identifier:
        identifier = _short_hash(f"{kind}:{row.get('name', '')}")
    return f"{prefix}_{_sbml_token(identifier)}_{_sbml_token(compartment_id)}"
...
def build_sbml(
    ...
    *,
    ...
    # ------------------------------------------------------------------
    preferred_mapped_keys = ["chebi", "hmdb", "kegg", "pubchem", "drugbank", "uniprot"]
    compound_rows_by_name = _dedupe_entity_rows(
        [row for row in _safe_list(entities.get("compounds")) if isinstance(row, dict)],
        preferred_mapped_keys=preferred_mapped_keys,
    )
    protein_rows_by_name = _dedupe_entity_rows(
        [row for row in _safe_list(entities.get("proteins")) if isinstance(row, dict)],
        preferred_mapped_keys=preferred_mapped_keys,
    )
    compound_alias_norms = {
        _normalize(alias): _normalize(row["name"])
        for alias, row in compound_rows_by_name.items()
        if _normalize(alias) and _normalize(alias) != _normalize(row["name"])
    }
    protein_alias_norms = {
        _normalize(alias): _normalize(row["name"])
        for alias, row in protein_rows_by_name.items()
        if _normalize(alias) and _normalize(alias) != _normalize(row["name"])
    }
    compound_rows = _unique_entity_rows(compound_rows_by_name)
    protein_rows = _unique_entity_rows(protein_rows_by_name)
    ...
    def _resolve_entity(name: str) -> Tuple[str, Optional[Dict[str, Any]]]:
        norm = _normalize(name)
        norm = compound_alias_norms.get(norm, protein_alias_norms.get(norm, norm))
        if norm in complex_by_norm:
    ...
    for loc_key, entity_field in [
        ...
        for row in _safe_list(element_locs.get(loc_key)):
            ...
            if ename and state:
                ...
                entity_states[compound_alias_norms.get(norm, protein_alias_norms.get(norm, norm))].append(state)
