...
if st.session_state.get("pipeline_ready"):
    ...
    if st.session_state.get("post_pipeline_artifacts"):
        if st.button("Generate PWML", key="refinement_generate_pwml"):
            result = {'ok': True, '_xml': '<pathway><name>menaquinone</name></pathway>', 'pwml_ir': {'pathway': {'name': 'menaquinone'}}, 'pwml_ir_report': {'counts': {'reactions': 1}}, 'validation_report': {'issues': 0}, 'qa': {'ok': True}, 'counts': {'reactions': 1, 'compounds': 2}, 'output_path': 'outputs/pathway.pwml'}
            xml = result.pop("_xml", "")
            if xml:
