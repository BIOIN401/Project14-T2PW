...
if st.session_state.get("pipeline_ready"):
    ...
    if st.session_state.get("post_pipeline_artifacts"):
        if st.button("Generate PWML", key="refinement_generate_pwml"):
            result = {'ok': False, 'error': 'Required-field gate rejected the payload', 'required_gate_report': {'ok': False, 'errors': [{'code': 'missing_external_identity', 'pointer': '/entities/proteins/0'}]}}
            xml = result.pop("_xml", "")
            if xml:
