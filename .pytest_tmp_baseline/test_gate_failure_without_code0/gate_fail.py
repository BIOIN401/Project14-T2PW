...
if st.session_state.get("pipeline_ready"):
    if st.button("Run audit and DB mapping", key="pwml_generate_btn"):
        st.session_state["post_pipeline_artifacts"] = {'gate_failed': False, 'normalization_gate_failed': True, 'gate_fail_report': {'status': 'failed', 'stage': 'post_normalization_hard_gates', 'error': 'Hard-gate validation failed after normalization.', 'errors': [{'path': '/entities/protein_complexes/0/name', 'reason': 'Forbidden complex reference detected: thyroglobulin:2-aminoacrylic acid'}, {'path': '/entities/protein_complexes/3/name', 'reason': 'Forbidden complex reference detected: beta-hydroxymyristoyl-ACP'}]}, 'export_mode': 'pathwhiz', 'research_review_flags': [], 'research_skipped_format_rules': [], 'research_normalization_actions': [], 'final_mapped_db': {'entities': {'compounds': [{'name': 'menaquinone'}, {'name': 'demethylmenaquinone'}], 'proteins': [{'name': 'MenG'}]}, 'processes': {'reactions': [{'name': 'menaquinone demethylation', 'inputs': ['menaquinone'], 'outputs': ['demethylmenaquinone']}]}}, 'final_mapped': {'entities': {'compounds': [{'name': 'menaquinone'}, {'name': 'demethylmenaquinone'}], 'proteins': [{'name': 'MenG'}]}, 'processes': {'reactions': [{'name': 'menaquinone demethylation', 'inputs': ['menaquinone'], 'outputs': ['demethylmenaquinone']}]}}, 'mapping_report': {'summary': {'mapped': 3}}, 'post_extraction_contract_report': {'ok': True, 'errors': [], 'warnings': []}}
