...
| Gold | `98739a59dd6c376f8a19968c7fa5dc3145be5b15` (**D-092**; `36f4b7b690b577f72882c3045ca6728d1ec8d9d1` before this wave. D-091 briefly moved it to `d0b588a79bb4aa3c11a7b5062a0b45bb8e20ab74` and was WITHDRAWN before merge — the flag is unset, the aliases ship) | `git hash-object src/t2pw/bench/gold/pinned_v1.json` |
...
|---|---|---|
| **`supported_reactions_complete`** | **AUDITED, SET, MEASURED and WITHDRAWN in `ORCH-722` — see D-091 then D-092.** The audit of `PMC12312563` is certified and stands; the flag does not. It charges the paper's own MenD reaction (a synonym, now fixed by aliases) and a correctly-attributed cross-paper RAG import from a paper this one CITES. **This is no longer a curation problem — it is blocked on `R-D092-1` (row-level RAG lineage) or a product-owner ruling on `R-D092-3`.** | **the only hard gate between a run and acceptance**, and the reason it stays shut has changed: not "nobody has audited a case" but "the scored corpus is not seed-only" |
| **F-175** | **DONE — `ORCH-722`, under a narrow product-owner D-090 exception for artifact persistence.** `driver.py` now carries `coverage_diagnostics.json` into the leg directory, resolved as a **sibling of the coverage report** — NOT as a tuple entry and NOT by widening the producer's returned map, both of which two pinned consumers assert by equality. Proved through a REAL batch leg and `runner.write_artifacts` onto disk; release record and `pathway.pwml` byte-identical. **The earlier row here called the fix "one tuple entry" — that was wrong on both counts and is corrected rather than deleted.** | a `D-089` requirement was unmet. **The test had to assert the file exists in a BATCH LEG DIRECTORY** — C-116 had eleven passing tests and none ran the batch path |
