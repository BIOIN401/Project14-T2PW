...
def main():
    ...
    print("D-088 STEP 6 (v3) — CURATED SET; REGULATORY STAGES ARE WARNINGS (clause 2)")
    print("=" * 96)
    print(f"repo                       : {repo}")
    ...
    for path in legs:
        ...
        for s in subs_l:
            ...
            if rids:
                ...
                # UNDETAILED subprocess: no reactions to match through, so it is
                # tested on its own name -- but only on tokens DISTINCTIVE to it.
                # Tokens of the requested pathway are removed first, because every
                # process in a pathway names that pathway and a test that accepts
                # them can never fail. See this module's header note; v1 scored
                # 'the upstream ... segment of cholesterol biosynthesis' as covered
                # on the token 'cholesterol'.
                st = toks(s.get("name")) - pathway_toks
